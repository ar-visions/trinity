...
image image_copy(image a) {
    ...
    memcpy(vdata((Au)res), vdata((Au)a), byte_count(a));
    return res;
}
...
image image_resize(image input, i32 out_w, i32 out_h) {
    ...
    u8* src = (u8*)vdata((Au)input);
    u8* dst = (u8*)vdata((Au)output);
...
A image_get(image a, num y) {
    i8* bytes = (i8*)vdata((Au)a);
    return (Au)&bytes[a->pixel_size * a->width * y];
}

define_class(image, Au);
...
none font_dealloc(font f) {
    font_manager_dealloc((Au)f);
}

A font_copy(font f) {
    font cp = (font)A_copy((Au)f);
    font_manager_init((Au)cp);
    return (Au)cp;
}

font font_init(font f) {
    font_manager_init((Au)f);
    return f;
}
