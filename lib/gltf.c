...
Transform Model_node_transform(Model data, Joints joints, m44f parent_mat, int node_index, Transform &parent) {
    ...
    if (node->joint_index >= 0) {
        ...
        each (node->children, object, p_node_index) {
            ...
            if (ch) {
                ...
                assert(n->joint_index == ch->istate);
...
Joints Model_joints(Model data, Node node) {
    if (node->mx_joints)
